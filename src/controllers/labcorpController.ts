...
export const labcorpController = {
    /**
     * Locate collection sites by zip code
     * POST /api/labcorp/locate-sites
     */
    async locateSites(req: AuthenticatedRequest, res: Response) {
        try {
            ...
            const sites = await locateCollectionSites(zip.trim(), searchDistance);
     ...
     */
    async selectLocation(req: AuthenticatedRequest, res: Response) {
        try {
            ...
            const {
                ...
                distance,
                phoneNumber,
                donorRegistrationId, // Optional: link to a donor registration
            } = req.body;

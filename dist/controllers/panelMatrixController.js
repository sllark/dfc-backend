...
exports.panelMatrixController = void 0;
const panelMatrixService_1 = require("../services/panelMatrixService");
const roles_1 = require("../utils/roles");
exports.panelMatrixController = {
    // ================= Panels =================
    async listPanels(req, res) {
        const panels = await panelMatrixService_1.panelMatrixService.getPanels();
    ...
    async getPanel(req, res) {
        ...
        const panel = await panelMatrixService_1.panelMatrixService.getPanelById(id);
        if (!panel)
    ...
    async createPanel(req, res) {
        ...
        const created = await panelMatrixService_1.panelMatrixService.createPanel({
            name,
            slug,
    ...
    async updatePanel(req, res) {
        ...
        if (isNaN(id))
            return res.status(400).json({ success: false, message: "Invalid panel id" });
        const updated = await panelMatrixService_1.panelMatrixService.updatePanel(id, req.body ?? {});
    ...
    async deletePanel(req, res) {
        ...
        if (isNaN(id))
            return res.status(400).json({ success: false, message: "Invalid panel id" });
        const updated = await panelMatrixService_1.panelMatrixService.deactivatePanel(id);
        res.json({ success: true, data: updated });
    },
    // ================= Test Items =================
    async listTestItems(req, res) {
        const items = await panelMatrixService_1.panelMatrixService.getTestItems();
        res.json({ success: true, data: items });
    },
    async getTestItem(req, res) {
        const id = Number(req.params.id);
        if (isNaN(id))
            return res.status(400).json({ success: false, message: "Invalid test item id" });
        const item = await panelMatrixService_1.panelMatrixService.getTestItemById(id);
        if (!item)
            return res.status(404).json({ success: false, message: "Test item not found" });
        res.json({ success: true, data: item });
    },
    async createTestItem(req, res) {
        (0, roles_1.requireAdmin)(req);
        ...
        const created = await panelMatrixService_1.panelMatrixService.createTestItem({
            name,
            slug,
    ...
    async updateTestItem(req, res) {
        (0, roles_1.requireAdmin)(req);
        const id = Number(req.params.id);
        if (isNaN(id))
            return res.status(400).json({ success: false, message: "Invalid test item id" });
        const updated = await panelMatrixService_1.panelMatrixService.updateTestItem(id, req.body ?? {});
        res.json({ success: true, data: updated });
    },
    async deleteTestItem(req, res) {
        (0, roles_1.requireAdmin)(req);
        const id = Number(req.params.id);
        if (isNaN(id))
            return res.status(400).json({ success: false, message: "Invalid test item id" });
        const updated = await panelMatrixService_1.panelMatrixService.deactivateTestItem(id);
        res.json({ success: true, data: updated });
    },
    // ================= Matrix =================
    async setIncluded(req, res) {
        (0, roles_1.requireAdmin)(req);
        ...
        const updated = await panelMatrixService_1.panelMatrixService.setIncluded(panelId, testItemId, Boolean(included));
        res.json({ success: true, data: updated });
    },
    // Public-friendly endpoint for website table
    async comparison(req, res) {
        const table = await panelMatrixService_1.panelMatrixService.getComparisonTable();
        res.json({ success: true, data: table });
    },
    // HTML preview that looks like the screenshot (server-rendered)
    async comparisonHtml(req, res) {
        const table = await panelMatrixService_1.panelMatrixService.getComparisonTable();
...
//# sourceMappingURL=panelMatrixController.js.map